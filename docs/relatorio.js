...
const capa = [
  ...
  new Paragraph({ spacing: { before: 800 }, alignment: AlignmentType.LEFT,
  ...
  new Paragraph({ spacing: { before: 1400 }, alignment: AlignmentType.CENTER,
    children: [run("Lubango, "), run(new Date().toLocaleDateString("pt-PT", { month: "long", year: "numeric" }))] }),
...
const corpo = [
  ...
  ...imagem("home.png", 380, 518, "Figura 1 — Página inicial do BASI, com demonstração da pesquisa semântica."),
  ...
  P("A Web Semântica estende a Web actual com significado processável por máquinas, assente num conjunto de normas do W3C:"),
  bullet([runB("RDF "), run("(Resource Description Framework): modelo de dados em triplos sujeito–predicado–objecto.")]),
  bullet([runB("RDFS / OWL: "), run("linguagens para descrever classes, propriedades e restrições, permitindo inferência.")]),
  bullet([runB("SPARQL: "), run("linguagem de consulta sobre grafos RDF; suporta caminhos de propriedade (por exemplo, eSubtemaDe* significa zero ou mais saltos).")]),
  bullet([runB("SKOS: "), run("vocabulário-padrão para representar sistemas de classificação e taxonomias, usado aqui para modelar a CDU.")]),
  ...
  P("Propriedades OWL utilizadas no projecto:"),
  ...
  ...imagem("pesquisa_resultados.png", 330, 600, "Figura 2 — Pesquisa semântica: termos expandidos por inferência e documentos encontrados."),
  ...
  ...imagem("biblioteca.png", 470, 406, "Figura 3 — Biblioteca organizada pelas secções da CDU."),
  ...
  sections: [
    ...
    // partir da Introdução.
    {
      ...
      footers: { default: new Footer({ children: [new Paragraph({
        alignment: AlignmentType.CENTER,
        children: [new TextRun({ text: "BASI — Semantic Academic Hub  |  Página ", size: 18 }),
