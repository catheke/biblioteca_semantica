...
const corpo = [
  ...
  H2("7.1 Como as pesquisas se associam entre si"),
  P("Uma pesquisa não devolve apenas a obra cujo título ou autor corresponde literalmente ao termo: o sistema liga cada termo a um tema do grafo e, a partir daí, percorre as relações entre temas para descobrir todas as obras associadas. São três os tipos de relação que tornam as pesquisas dependentes umas das outras:"),
  bullet([runB("Tema → subtema "), run("(basi:eSubtemaDe, transitiva): liga um tema aos seus temas mais específicos. Pesquisar um tema associa automaticamente todos os seus descendentes [3].")]),
  bullet([runB("Documento → tema "), run("(basi:temTema): liga cada obra aos temas que aborda; é a ponte entre a pesquisa por tema e as obras concretas.")]),
  bullet([runB("Obra ↔ obra "), run("(basi:obraRelacionada, simétrica): liga directamente obras afins, reforçando as recomendações.")]),
  P("O grafo de temas em torno de «Inteligência Artificial» tem a forma seguinte — e é esta estrutura que explica por que razão uma pesquisa por IA também encontra obras de Deep Learning ou de Big Data, ainda que estas palavras não constem do termo procurado:"),
  ...code([
  ...
  P("A relação entre pesquisas é, assim, explícita e navegável: «Big Data» é subtema de «Ciência de Dados», que por sua vez é subtema de «Inteligência Artificial». Uma pesquisa por qualquer um destes termos associa-se aos restantes através do caminho eSubtemaDe*, e cada resultado indica o tema que o tornou relevante — tornando visível a ligação entre a pesquisa e a obra encontrada. É esta cadeia de relações (termo → tema → subtemas → documentos → obras relacionadas) que distingue a pesquisa semântica de uma simples pesquisa por título ou autor."),
